...
@interface YIMC2CChatViewController ()
...
@implementation YIMC2CChatViewController
...
- (void)inputController:(TUIInputController *)inputController didSelectMoreCell:(TUIInputMoreCell *)cell
{
    ...
    else if([key isEqualToString:YIMInputMoreCellKeyOrder]) {  // 自定义消息
        NSString *productImageURLString = @"https://img14.360buyimg.com/n0/jfs/t1/71519/15/23235/87646/637f19daE117c1f6c/efe9716f210a4fe6.jpg";
        NSString *productName = @"浦林(Prinx)轮胎";
        NSError *error = nil;
        NSDictionary *param = @{BussinessID: YIMMessageBussinessIDOrder, @"productImageURLString":productImageURLString, @"productName":productName};
        NSData *data = [NSJSONSerialization dataWithJSONObject:param options:0 error:&error];
        if(error)
        {
            NSLog(@"[%@] Post Json Error", [self class]);
...
- (void)messageController:(TUIBaseMessageController *)controller onSelectMessageContent:(TUIMessageCell *)cell
{
    ...
    if ([cell isKindOfClass:[YIMOrderMessageCell class]]) {
        [self onSelectOrderCell:(YIMOrderMessageCell *)cell];
    }
}

- (void)onSelectOrderCell:(YIMOrderMessageCell *)cell {
    YIMOrderMeaageCellData *cellData = cell.customData;
    if (cellData.productImageURLString) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:cellData.productImageURLString] options:@{} completionHandler:nil];
    }
}
