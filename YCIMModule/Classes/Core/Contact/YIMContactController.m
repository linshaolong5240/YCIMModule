...
@interface YIMContactController () <UITableViewDelegate, UITableViewDataSource, V2TIMFriendshipListener>

@property NSArray<TUIContactActionCellData *> *firstGroupData;

@end

@implementation YIMContactController
...
- (void)viewDidLoad {
    ...
    [list addObject:({
        ...
        data.icon = [YIMImage imageNamed:@"avatar_new_friend"];//TUIContactDynamicImage(@"contact_new_friend_img", [UIImage imageNamed:TUIContactImagePath(@"new_friend")]);
        data.title = @"新的车友";//TUIKitLocalizableString(TUIKitContactsNewFriends);
    ...
    [_tableView registerClass:[TUICommonContactCell class] forCellReuseIdentifier:NSStringFromClass([TUICommonContactCell class])];
    [_tableView registerClass:[TUIContactActionCell class] forCellReuseIdentifier:NSStringFromClass([TUIContactActionCell class])];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        TUIContactActionCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([TUIContactActionCell class]) forIndexPath:indexPath];
        [cell fillWithData:self.firstGroupData[indexPath.row]];
        cell.changeColorWhenTouched = YES;
        return cell;
    } else {
        TUICommonContactCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([TUICommonContactCell class]) forIndexPath:indexPath];
